...
def profiles():
    ...
    for user in users:
        profiles.append({"pro_pic": user.photo, "f_name":user.firstname, "l_name": user.lastname, "gender": user.gender, "lcoation":user.location, "id":user.id})

    return render_template("view_all_profiles.html", profile = profiles)


@app.routes("/profile/<userid>")
